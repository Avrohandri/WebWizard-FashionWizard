...
$(document).ready(function () {
	...
	$.ajax({
	  ...
	  success: function (data) {
		...
		let lastSale = salesData[salesData.length - 1] || 0;
		predictedData = salesData.map((sale, index) => {
		  let factor = 1 + (Math.sin((index + 1) * 0.5) * 0.1); // Example prediction factor
		  return Math.round(lastSale * factor);
		});
		...
		// Update current sale
		let currentSale = salesData[salesData.length - 1] || 0;
		$('#currentSale').text(`Rp ${currentSale.toLocaleString('id-ID')}`);
  
		// Predict next month sales using a simple algorithm
		let totalSales = salesData.reduce((a, b) => a + b, 0);
		let predictedSales = totalSales / salesData.length;
		$('#prediction').text(`Rp ${predictedSales.toLocaleString('id-ID')}`);
	  },
	  error: function (err) {
		console.log(err);
	  }
	...
	$.ajax({
	  url: '/api/category-sales',
	  method: 'GET',
	  success: function (data) {
		let labels = data.map(item => item.category);
		let salesData = data.map(item => item.sales);
		...
		console.log("Chart created:", categorySalesChart);
	  },
	  error: function (err) {
		console.log("Error fetching category sales data:", err);
	  }
	});
  });
